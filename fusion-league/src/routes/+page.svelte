<script lang="ts">
  ...
  import { getPicksWindowStatus } from "$lib/scoring/weeks";
  ...
  onMount(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "player_scores", filter: `week_id=eq.${data.week.id}` },
        () => {
...
<svelte:head><title>Fusion League — Leaderboard</title></svelte:head>
...
<div class="page">
  ...
  {:else}
    <table class="leaderboard">
      <thead>
        <tr>
          <th>#</th><th>Manager</th><th>Tank</th><th>Healer</th><th>DPS</th><th>Score</th><th>Status</th>
        </tr>
      </thead>
      <tbody>
        {#each leaderboard as entry, i}
          <tr class:row--top={i === 0}>
            ...
            <td
              >{entry.roster[2]?.character_name ?? "—"}, {entry.roster[3]?.character_name ?? "—"}, {entry.roster[4]
                ?.character_name ?? "—"}</td
            >
